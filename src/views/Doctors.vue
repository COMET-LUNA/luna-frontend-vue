  ...
  <div class="row px-5">
    ...
    <div class="col-9" v-else>
      <div class="mt-4">
        ...
        />
        <span v-if="firstRecommendations.length == 0"
          >No doctors matches all your preferences.</span
        >
      </div>
      <div class="mt-4">
        <h3 class="mb-3 text-start">
        ...
        <DoctorRow
          ...
          @click="showModal(doctor)"
        />
        <span v-if="secondRecommendations.length == 0"
          >No doctors matches your location preference and specialization.</span
        >
      </div>
      <div class="mt-4">
        <h3 class="mb-3 text-start">
          Doctors who specialize in {{ specialization }}
        </h3>
        <div v-if="specRecommendations.length > 0">
          <div v-if="showSpec" class="mb-5">
            <DoctorRow
              v-for="(doctor, index) in specRecommendations"
              :doctor="doctor"
              :isFirst="false"
              :key="index"
              @click="showModal(doctor)"
        ...
        <span v-if="specRecommendations.length == 0"
          >No doctors matches your recommended specialization.</span
        >
      </div>
    </div>
  </div>
